...
STAGING_DIR=/tmp/hbase-stream-staging
...
# ── Step 5: Run Spark Streaming load ─────────────────────────────────────────
echo ""
echo "=== Step 5: Running Spark Streaming load (table=$TABLE_NAME) ==="
exec spark-shell \
  --conf "spark.driver.extraClassPath=$HB_CP" \
  --conf "spark.executor.extraClassPath=$HB_CP" \
  --conf "spark.hbase.streaming.tableName=$TABLE_NAME" \
  <<'REPL'
:load /scripts/stream-dengue.scala
REPL
